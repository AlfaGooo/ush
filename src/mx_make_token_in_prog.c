...
static void end_argv(int *curr_pos, int end, char *str) {
    ...
    while (mx_check_char(str[*curr_pos]) || (str[*curr_pos] == ' '
...
t_token *mx_make_token_in_prog(int *curr_pos, int end, char *str,
                                 t_info *processes) {
    int token_start = *curr_pos;
    t_token *new_token = mx_make_token(2,0,0);
    char *newValue = 0;
    char *temp = 0;

    while (mx_check_char(str[token_start])) {
        end_argv(curr_pos, end, str);
        newValue = mx_strndup(&str[token_start], *curr_pos - token_start);
        temp = mx_check_str(newValue, processes, 0);
        mx_strdel(&newValue);
