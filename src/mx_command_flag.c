...
static bool flag_oper(char *str, int *pos, int flag) {
    ...
    if (str[pos[0]] == '|' || str[pos[0]] == '<'
        || str[pos[0]] == '>' || str[pos[0]] == '&')
        if (mx_is_symbol_in_position(str, pos[0], str[pos[0]])) {
            pos[0]--;
            return 1;
        }
    return 0;
...
int mx_command_flag(char *str, int *pos, int end, int flag) {
    ...
    while (pos[0] <= end) {
        ...
        if (mx_is_symbol_in_position(str, pos[0], flag) || flag_oper(str, pos, flag)) {
            return 0;
        }
        else if ((if_symbol(str[*pos])
                  && mx_is_symbol_in_position(str, pos[0], str[pos[0]]))
