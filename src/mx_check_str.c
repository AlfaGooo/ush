...
static void editor_str(char **str, t_info *processes) {
    ...
    if (str[0][0] == 39) {
        ...
        *str = temp;
        ...
        temp2 = mx_check_str(temp, processes, 1);
        mx_subs(&temp2);
        mx_strdel(str);
        mx_strdel(&temp);
        *str = temp2;
...
static bool chek_comand(char *new_str, int i) {
    if (new_str[i] == 34 || new_str[i] == 39 || new_str[i] == 96) {
        if (i == 0 || new_str[i - 1] != '\''
            || !mx_is_symbol_in_position(new_str, i, new_str[i]))
            return true;
        else
            return false;
    }
        ...
        return true;
             ...
             && !mx_is_symbol_in_position(new_str, i, '$'))
        return true;
    else
        return false;
}
...
char *mx_check_str(char *str, t_info *info, bool dqute) {
    char *new_str = 0;
    int i = 0;
    int pos = 0;
